...
def capri_class(fnat, iRMS, LRMS, capri_peptide=False):
    if capri_peptide:
        if fnat < 0.2 or (LRMS > 5.0 and iRMS > 2.0):
            ...
            (fnat >= 0.2 and fnat < 0.5) and (LRMS <= 5.0 or iRMS <= 2.0) or (fnat >= 0.5 and LRMS > 2.0 and iRMS > 1.0)
        ):
            return "Acceptable"
        elif (
            (fnat >= 0.5 and fnat < 0.8) and (LRMS <= 2.0 or iRMS <= 1.0) or (fnat >= 0.8 and LRMS > 1.0 and iRMS > 0.5)
        ):
            return "Medium"
        ...
        if fnat < 0.1 or (LRMS > 10.0 and iRMS > 4.0):
            ...
            (fnat >= 0.3 and fnat < 0.5) and (LRMS <= 5.0 or iRMS <= 2.0) or (fnat >= 0.5 and LRMS > 1.0 and iRMS > 1.0)
        ):
            return "Medium"
...
def main():
    ...
    parser.add_argument(
        "-skip_check", default=False, action="store_true", help="skip initial check fo speed up on two chain examples"
    )
    parser.add_argument(
        "-no_needle",
        default=False,
    ...
    parser.add_argument(
        "-model_chain1", metavar="model_chain1", type=str, nargs="+", help="pdb chain order to group together partner 1"
    )
    parser.add_argument(
        "-model_chain2",
    ...
    #    print native_chains
    if (len(model_chains) > 2 or len(native_chains) > 2) and (args.model_chain1 == None and args.native_chain1 == None):
        print(
